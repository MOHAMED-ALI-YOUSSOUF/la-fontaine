...
const ContactForm = () => {
    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            const emailJsResult = await emailjs.send('service_x4buxmd', 'template_aabvqnb', templateParams, 'g3chyHIhdc9qZ1Ep5');
    
            if (emailJsResult.status === 200) {
                formik.resetForm();
                swalAlert("Your message was sent", "success");
            } else {
                swalAlert("Failed to send your message", "error");
            }
        } catch (error) {
            swalAlert("An error occurred while sending the message", "error");
        } finally {
            setLoading(false);
        }
    }
